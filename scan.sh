...
# =====================
# 🔗 WIFI DEBUG / PAIRING RECENTE (MELHORADO E RESUMIDO)
# =====================
echo ""
echo "🔗 [WIFI DEBUG / PAIRING RECENTE]"

LOGCAT=$(logcat -d 2>/dev/null)
PAIR_LOGS=$(echo "$LOGCAT" | grep -iE "AdbDebuggingManager|pairing|unpair|forget|remove.*device|paired.*device|device.*removed|pair.*code|Wireless Debug" | tail -n 20)

pairing_flags=0

if [ -n "$PAIR_LOGS" ]; then
  echo "📋 Eventos de Pareamento/Desparelhamento detectados:"
  echo "$PAIR_LOGS" | while read -r line; do
    # Extrai data/hora e resume a linha
    timestamp=$(echo "$line" | awk '{print $1 " " $2}')
    message=$(echo "$line" | cut -d']' -f2- | sed 's/^[ \t]*//')
    
    if echo "$line" | grep -qiE "unpair|forget|remove|delete|apagado"; then
      echo "   🟥 [DESPARELHADO] $timestamp → $message"
      pairing_flags=$((pairing_flags+7))
    else
      echo "   🟨 [PAREADO]     $timestamp → $message"
      pairing_flags=$((pairing_flags+4))
    fi
  done
else
  echo "✅ Nenhum pareamento ou desparelhamento recente detectado nos logs"
fi

# Avaliação de score
if [ $pairing_flags -ge 7 ]; then
  echo "🚨 Dispositivo foi desparelhado recentemente (suspeito)"
  score=$((score+9))
...
clear
reset
echo "Terminal limpo e resetado."  score=$((score+3))
else
  echo "✅ Kernel padrão"
fi
