...
function scanCompleto(string $pacote = 'com.dts.freefireth', string $nomeJogo = 'Free Fire'): void {
    ...
    $pastasCriticas = ["$pacoteDir/gameassetbundles", "$pacoteDir/optionalavatarres"];
    ...
    foreach ($pastasCriticas as $pasta) {
        ...
        if ($modify !== $change) {
            erro("WALLHACK DETECTADO → Modificação suspeita em:");
            detalhe($pasta);
            file_put_contents($RESULT_FILE, "[WALLHACK] $pasta\n", FILE_APPEND);
    ...
    if (adb("shell \"test -d '$mreplaysDir' && echo found\"") === 'found') {
        ...
        if (strpos($permCheck, 'Permission denied') !== false) {
            erro("MREPLAYS → Permissão removida (bypass detectado)");
            file_put_contents($RESULT_FILE, "[MREPLAYS] Permissão removida\n", FILE_APPEND);
            $mreplaysDetectado = true;
            $score += 25;
        }

        $binFiles = adb("shell \"ls -lt '$mreplaysDir'/*.bin 2>/dev/null | head -6\"");
        if (!empty(trim($binFiles))) {
            erro("ARQUIVOS .bin EM MREPLAYS (Replay passado detectado)");
            file_put_contents($RESULT_FILE, "[MREPLAYS] Arquivos .bin encontrados\n", FILE_APPEND);
            $mreplaysDetectado = true;
            $score += 20;
        }
    }
    ...
    // Resumo Final
    secao(99, "RESUMO FINAL");

    if ($score >= 60) {
        erro("💀 CRÍTICO - ALTO RISCO DE CHEAT");
    } elseif ($score >= 40) {
    ...
    echo "\n" . c('bold', 'branco') . "  Score: $score\n";
    echo "  Relatório: /sdcard/resultSCAN.txt\n" . rst();
...
function exibirMenu(): void {
    system('clear');
    kellerBanner();
    echo c('bold', 'azul') . "  [1] Free Fire Normal\n";
    echo c('bold', 'azul') . "  [2] Free Fire MAX\n";
...
while (true) {
    ...
    if ($op === '1') scanCompleto('com.dts.freefireth', 'Free Fire Normal');
    elseif ($op === '2') scanCompleto('com.dts.freefiremax', 'Free Fire MAX');
    elseif ($op === '0') {
        echo "\nObrigado!\n";
        exit(0);
    }
